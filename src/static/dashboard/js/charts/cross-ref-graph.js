...
function renderCrossRefGraph(indexData) {
  ...
  node.append('circle')
    ...
    .attr('fill', d => TYPE_COLORS[d.type] || css('--text-dim'))
    .attr('fill-opacity', 0.85)
    .attr('stroke', css('--accent-15'))
    .attr('stroke-width', 1);
  ...
  node.on('mouseenter', function(event, d) {
    ...
    link.attr('stroke', l =>
      (l.source.id === d.id || l.target.id === d.id) ? TYPE_COLORS[d.type] || css('--accent') : css('--accent-03')
  ...
  .on('mouseleave', function() {
    ...
    link.attr('stroke', css('--accent-06')).attr('stroke-opacity', 1);
  });
